...
// });
var timer;
$('.category-menu-toggle-btn').on('mouseenter', function () {
    timer = setTimeout(function () {
        $('.megamenu-wrap').addClass('show');
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.errors) {
                ...
                //refresh captcha
                let $captcha = $("#default_recaptcha_id");
                if ($captcha.length) {
                    let src = $captcha.attr("src").split("?")[0];
                    let sessionId = "default_recaptcha_id_customer_regi";
                    $captcha.attr("src", src + "?captcha_session_id=" + sessionId + "&_=" + new Date().getTime());
...
$(".clickable").click(function (e) {
    ...
    if ($(e.target).closest('.wishlist-btn, .product-action-add-wishlist-single, .wishlist-btn-container').length > 0) {
        return false; // Don't redirect if wishlist button was clicked
    }
...
// Use event delegation for dynamically loaded wishlist buttons
$(document).on('click', '.wishlist-btn, .product-action-add-wishlist-single, .product-action-add-wishlist-single', function (e) {
    e.stopPropagation(); // Prevent event bubbling
    e.preventDefault(); // Prevent default action
    const product_id = $(this).data('product-id');
    const modalId = $(this).data('modal-id');
    addWishlist(product_id, modalId);
});

// Handle wishlist button clicks with stopPropagation
$(document).on('click', '.product-action-add-wishlist-single', function (e) {
    e.stopPropagation(); // Prevent event bubbling
    let id = $(this).data('product-id');
    addWishlist(id);
});

function addWishlist(product_id, modalId, callback) {
    $.ajaxSetup({
        headers: { 'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content') }
    });
    $.ajax({
        ...
        success: function (data) {
            if (data.value == 1) {
                ...
                $('.tooltip').html('');
                $(`.wishlist_icon_${product_id}`).removeClass('fa fa-heart-o').addClass('fa fa-heart');
                $(`.wishlist-btn[data-product-id=\"${product_id}\"] i`).removeClass('fa-heart-o').addClass('fa-heart');
                $(`.wishlist-btn[data-product-id=\"${product_id}\"]`).addClass('activeheart');
                toastr.success(data.success || 'Product has been added to wishlist');
            } else if (data.value == 2) {
                $('.countWishlist').html(data.count);
                $('.countWishlist-' + product_id).text(data.product_count);
                $(`.wishlist_icon_${product_id}`).removeClass('fa fa-heart').addClass('fa fa-heart-o');
                $(`.wishlist-btn[data-product-id=\"${product_id}\"] i`).removeClass('fa-heart').addClass('fa-heart-o');
                $(`.wishlist-btn[data-product-id=\"${product_id}\"]`).removeClass('activeheart');
                toastr.info(data.error || 'Product removed from wishlist');
            } else {
                toastr.error(data.error || 'Unable to add to wishlist.');
                $('#login-alert-modal').modal('show');
            }
        ...
        $.post({
            ...
            success: function (response) {
                ...
                if (response.status == 1) {
                    ...
                    // if (redirect_to_checkout) {
                    //     location.href = "{{ route('checkout.details') }}"; // Blade syntax
                    // } else {
                    //     location.reload(); // Reload the page if not redirecting
                    // }
                    // location.reload();
                    return false;
                } else if (response.status == 0) {
...
function commonFunctionalityForProductView() {
    ...
    $('.product-action-add-wishlist-single').on('click', function () {
        let id = $(this).data('product-id');
        addWishlist(id);
    })
}
...
function handleVibration(element) {
    element.classList.add('vibrate');
    setTimeout(() => {
        element.classList.remove('vibrate');
    }, 500);
}
// Set up event listeners for both classes
document.querySelectorAll('.function-remove-wishList, .product-action-add-wishlist-single').forEach(button => {
    button.addEventListener('click', function () {
        handleVibration(this);
    });
});
// For dynamically added elements (if needed)
document.addEventListener('click', function (e) {
    if (e.target.closest('.function-remove-wishList, .product-action-add-wishlist-single')) {
        handleVibration(e.target.closest('.function-remove-wishList, .product-action-add-wishlist-single'));
    }
});

// Animation for wishlist ends
