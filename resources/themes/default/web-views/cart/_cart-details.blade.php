                                    ...
                                    <div class="d-flex gap-3">
                                        ...
                                        <div class="d-flex flex-column gap-1">
                                            <div
                                                ...
                                                <a href="{{ $checkProductStatus == 1 ? route('product',$cartItem['slug']) : 'javascript:'}}" class="cart-item-name">{{$cartItem['name']}}</a>
                                            </div>
                                            <div
        ...
        @if($shippingMethod=='inhouse_shipping')
            ...
            @if ($shipping_type == 'order_wise' && $isPhysicalProductExist)
                    ...
                    <div class="row">
                        <div class="col-12">
                            <select class="form-control border-aliceblue action-set-shipping-id"
                                ...
                                <option value=""  {{ empty($chosenShipping['shipping_method_id']) ? 'selected' : '' }}>
                                ...
                                @foreach($shippings as $shipping)
                                    <option
                                        ...
                                        {{ translate('shipping_method') }}: {{ $shipping['title'].' ( '.$shipping['duration'].' ) '.webCurrencyConverter(amount: $shipping['cost']) }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                ...
                {{-- <div class="px-3 px-md-0 mb-3">
                    <div class="row">
                        <div class="col-12">
                            <div class="shipping-method-options" data-cart-group="{{ $cartItem['cart_group_id'] }}">
                                @foreach($shippings as $shipping)
                                <div class="form-check mb-2">
                                    <input class="form-check-input shipping-method-radio"
                                        type="radio"
                                        name="shipping_method_{{ $cartItem['cart_group_id'] }}"
                                        id="shipping_{{ $shipping['id'] }}_{{ $cartItem['cart_group_id'] }}"
                                        value="{{ $shipping['id'] }}"
                                        checked
                                        data-cost="{{ $shipping['cost'] }}"
                                        data-title="{{ $shipping['title'] }}"
                                        data-duration="{{ $shipping['duration'] }}">
                                    <label class="form-check-label" for="shipping_{{ $shipping['id'] }}_{{ $cartItem['cart_group_id'] }}">
                                        {{ $shipping['title'] }} ({{ $shipping['duration'] }}) - {{ webCurrencyConverter(amount: $shipping['cost']) }}
                                    </label>
                                </div>
                                @endforeach
