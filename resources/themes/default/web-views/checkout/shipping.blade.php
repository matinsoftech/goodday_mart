...
@endpush
<style>
    .select_shipping_address.active {
        border-radius: 8px;
                    ...
                    @if ($physical_product_view)
                        ...
                        <div class="d-flex align-items-center justify-content-between">
                            <div class="px-3 px-md-0">
                                <h4 class="pb-2 mt-4 fs-18 text-capitalize checkout-details-title">
                                    {{ translate('shipping_address') }}
                                </h4>
                            </div>
                            <div class="px-3 px-md-0">
                                <a href="{{ url('account-address') }}"
                                    class="pb-2 mt-4 fs-18 text-capitalize checkout-add-address">+ Add
                                    Address</a>
                            </div>
                        </div>
                        @php($shippingAddresses = \App\Models\ShippingAddress::where(['customer_id' => auth('customer')->id(), 'is_billing' => 0, 'is_guest' => 0])->get())
                        <form method="post" class="card __card bg-unset" id="address-form">
                            <div class="card-body p-2">
                                <!-- new address list design -->
                                @if ($shippingAddresses->count() > 0)
                                    <div class="d-flex align-items-center flex-wrap">
                                        @foreach ($shippingAddresses as $key => $address)
                                            <div class="col-sm-6 col-12 mb-1">
                                                <div class="dropdown-item select_shipping_address checkout-shippingAddress {{ $key == 0 ? 'active' : '' }}"
                                                    ...
                                                    <div class="">
                                                        ...
                                                        <div class="fs-12 text-capitalize text-wrap">City:
                                                            {{ $address->city }}
                                                        </div>
                                                        <div class="fs-12 text-capitalize text-wrap">Country:
                                                            {{ $address->country }}
                                                        </div>
                                                        <div class="fs-12 text-capitalize text-wrap">Address:
                                                            {{ $address->address }}
                                                        </div>
                                                    </div>
                                                </div>
                                                        ...
                                                        <div class="col-12">
                                                            <div class="form-group">
                                                                ...
                                                                <select name="country" id="country"
                                                                    ...
                                                                    @forelse($countries as $country)
                                                                        <option value="{{ $country['name'] }}"
                                                                            {{ isset($selectedCountry) && $selectedCountry == $country['name'] ? 'selected' : '' }}>
                                                                            {{ $country['name'] }}
                                                                        </option>
                                                                    @empty
                                                                        <option value="">
...
@push('script')
    ...
    <script
        src="https://maps.googleapis.com/maps/api/js?key={{ getWebConfig(name: 'map_api_key') }}&callback=mapsShopping&libraries=places&v=3.49"
        defer></script>
@endpush
