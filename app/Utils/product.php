...
if (!function_exists('getPriceRangeWithDiscount')) {
    function getPriceRangeWithDiscount(array|object $product): float|string
    {
        ...
        if ($product->discount > 0) {
            ...
            return '<span class="discounted_unit_price fs-24 font-bold details-page-theme-col">' . $productDiscountedPrice . '</span>' . '<del class="total_unit_price align-middle text-muted fs-18 font-semibold details-page-theme-col">' . webCurrencyConverter(amount: $productUnitPrice) . '</del>';
        } else {
            return '<span class="discounted_unit_price fs-24 font-bold details-page-theme-col">' . webCurrencyConverter(amount: $productUnitPrice) . '</span>';
        }
    }
}
