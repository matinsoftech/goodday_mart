...
class CartManager
{
    ...
    public static function add_to_cart($request, $from_api = false)
    {
        ...
        if ($product->choice_options) {
            foreach (json_decode($product->choice_options) as $choice) {
                if (!isset($request[$choice->name])) {
                    return [
                        'status' => 0,
                        'message' => translate('missing_variation_options')
                    ];
                }
                $choices[$choice->name] = $request[$choice->name];
                $variations[$choice->title] = $request[$choice->name];
                $str .= ($str ? '-' : '') . str_replace(' ', '', $request[$choice->name]);
            }
        }
        ...
        if ($str) {
            ...
            foreach ($variations as $variation) {
                if ($variation->type == $str) {
                    ...
                    if ($variation->qty < $request->quantity) {
                        return [
                            'status' => 0,
                            'message' => translate('out_of_stock!')
                        ];
            ...
            if (!$found) {
                return [
                    'status' => 0,
                    'message' => translate('invalid_variation')
                ];
            }
        } else {
            $price = $product->unit_price;
        ...
        // Calculate tax and other fields
        $tax = Helpers::tax_calculation($product, $price, $product->tax, 'percent');

        // Set cart group ID
        $cartGroup = Cart::where([
            'customer_id' => $user == 'offline' ? $guest_id : $user->id,
            'is_guest' => $user == 'offline' ? 1 : 0,
            'seller_id' => $product->added_by == 'admin' ? 1 : $product->user_id,
        ...
        $cart->cart_group_id = $cartGroup
            ? $cartGroup->cart_group_id
            : ($user == 'offline' ? 'guest' : $user->id) . '-' . Str::random(5) . '-' . time();
        ...
        $cart->shipping_cost = $product->product_type == 'physical'
            ...
            : 0;
        $cart->shop_info = $product->added_by == 'seller'
        ...
        if ($shippingMethod == 'inhouse_shipping') {
            $admin_shipping = ShippingType::where('seller_id', 0)->first();
            $cart->shipping_type = $admin_shipping ? $admin_shipping->shipping_type : 'order_wise';
        } else {
            $shipping = $product->added_by == 'admin'
                ? ShippingType::where('seller_id', 0)->first()
                : ShippingType::where('seller_id', $product->user_id)->first();
            $cart->shipping_type = $shipping ? $shipping->shipping_type : 'order_wise';
        }
    ...
    public static function update_cart_qty($request)
    {
        ...
        if (!$cart) {
            return [
                'status' => 0,
                'qty' => 0,
                'message' => translate('cart_item_not_found')
            ];
        }
        ...
        // Update if stock available
        if ($status) {
            $qty = $request->quantity;
            $cart->quantity = $qty;
            $cart->shipping_cost = $product->product_type == 'physical'
                ? CartManager::get_shipping_cost_for_product_category_wise($product, $qty)
                : 0;
        ...
        return [
            ...
            'message' => $status == 1
                ...
                : translate('sorry_stock_is_limited')
        ];
    }
