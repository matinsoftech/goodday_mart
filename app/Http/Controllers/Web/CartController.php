...
class CartController extends Controller
{
    ...
    public function variant_price(Request $request)
    {
        ...
        // Debug: Ensure product is fetched correctly
        ...
        // Construct variation string from color and choice options
        ...
        // Debug: Check the constructed variation code

        dd('Constructed variation code:', $str);
        ...
        // Check product variation to find the matching one
        ...
        if ($variations) {
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                if ($variations[$i]->type == $str) {
                    ...
                    $tax = $product->tax_model == 'exclude' ? Helpers::tax_calculation(product: $product, price: $variations[$i]->price, tax: $product['tax'], tax_type: $product['tax_type']) : 0;

                    $update_tax = $tax * $requestQuantity;

                    $discount = Helpers::get_product_discount($product, $variations[$i]->price);

                    $price = $variations[$i]->price - $discount + $tax;

                    $discountedUnitPrice = $variations[$i]->price - $discount;

                    $unit_price = $variations[$i]->price;

                    $quantity = $variations[$i]->qty;

                    // Debug: Check the price calculation

                    dd('Price calculation:', [
            ...
            // Debug: Ensure variations exist

            dd('No variations found for product');
        }

        // If no specific variation is found, use the base product price
        ...
        if ($str == null) {
            ...
            // Debug: Check the base price calculation

            dd('Base price calculation:', [
        ...
        return [
            ...
            'update_tax' => $product->tax_model == 'exclude' ? \App\Utils\Helpers::currency_converter($update_tax) : 'incl.',
            ...
            'unit_price' => \App\Utils\Helpers::currency_converter($price),

            'total_unit_price' => \App\Utils\Helpers::currency_converter($unit_price),

            'discounted_unit_price' => \App\Utils\Helpers::currency_converter($discountedUnitPrice),
    ...
    public function updateCartQuantity(Request $request)
    {
        ...
        $customerId = auth('customer')->check()
            ? auth('customer')->id()
            : session('guest_id');
    ...
    public function deleteItems(Request $request)
    {
        ...
        $deleted = Cart::where('customer_id', $customerId)
            ->where('product_id', $request->product_id)
            ->delete();
